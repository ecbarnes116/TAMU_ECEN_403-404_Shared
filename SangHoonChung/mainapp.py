...
class MainApp(App):# Define a custom class named MainApp that inherits from Kivy's App class.
    ...
    def calculate_statistics(self, df):# Calculate statistics of a given DataFrame and return the results.
        ...
        if max_value < 200:
            ...
            explosion_time_index = np.where(signal > 200)[0][0]
            explosion_time = df.loc[explosion_time_index, 'time']
            last_time_index = np.where(signal[explosion_time_index+1:] == 0)[0]
            if len(last_time_index) > 0:
                explosion_time_end_index = last_time_index[0] + explosion_time_index + 1
                explosion_time_end = df.loc[explosion_time_end_index, 'time']
                explosion_time_delta = pd.Timestamp(explosion_time_end) - pd.Timestamp(explosion_time)
                explosion_time_seconds = explosion_time_delta.total_seconds()
                explosion_time = f"{explosion_time} - {explosion_time_end} ({explosion_time_seconds:.2f} seconds)"
            else:
                explosion_time = "N/A"
